...
const SingleSeriesBanner = ({ series, individual, trailer, show }) => {
  return (
    <div key={series.id} className='h-[90vh] focus-visible:outline-none relative bg-bannerImg '>
      <div className='absolute h-[90vh] inset-0 bg-bannerImg'></div>
      <Image priority width={1920} height={1200} className='h-[90vh] w-full object-cover focus-visible:border-none' src={requests.imgBase + series.backdrop_path} alt={series.name || series.original_name} />
      <div className='absolute bottom-[80px] text-white px-3 pl-[28px] sm:pl-[36px] md:pl-[48px] lg:pl-[60px] w-full'>
        <h2 className='text-center sm:text-left font-bold text-xl sm:text-3xl md:text-4xl lg:text-6xl xl:text-7xl sm:w-[30ch] mb-3'>{series.name || series.original_name}</h2>
        <div className='flex justify-center sm:justify-start gap-4 text-gray-400 mb-1'>
          {show ? <p>{series.number_of_seasons} Temporadas</p> : <p>{series.episode_run_time} MIN</p>}
          <p>{series.release_date?.slice(0, 4)}</p>
        </div>
        <p className={`text-center sm:text-left max-w-[800px] sm:pr-4 sm:mb-6 ${individual ? '' : ' parrafo text-ellipsis overflow-hidden sm:whitespace-nowrap'}  md:text-lg  `}>{series.overview}</p>
