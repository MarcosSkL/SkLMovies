...
const SingleSeriesBanner = ({ serie, individual, trailer, show }) => {
  return (
    <div key={serie.id} className='h-[90vh] focus-visible:outline-none relative bg-bannerImg '>
      ...
      <div className='absolute bottom-[80px] text-white px-3 pl-[28px] sm:pl-[36px] md:pl-[48px] lg:pl-[60px] w-full'>
        <h2 className='text-center sm:text-left font-bold text-xl sm:text-3xl md:text-4xl lg:text-6xl xl:text-7xl sm:w-[30ch] mb-3'>{serie.name || serie.original_name}</h2>
        <div className='flex justify-center sm:justify-start gap-4 text-gray-400 mb-1'>
          {show ? <p>{serie.number_of_seasons} Temporadas</p> : <p>{serie.episode_run_time} MIN</p>}
