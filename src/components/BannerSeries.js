...
const BannerSeries = ({ popularMovies, individual, series, trailer, show }) => {
  ...
  return (
    <div className='group'>
      <Slider {...settings}>
        {individual ? (
          <SingleSeriesBanner series={series} show={show} individual={individual} trailer={trailer} />
        ) : (
          popularMovies.map((item) => {
            return (
              <div key={item.id} className='h-[90vh] focus-visible:outline-none relative bg-bannerImg '>
                ...
                <Image priority width={1920} height={1200} className='h-[90vh] w-full object-cover focus-visible:border-none' src={requests.imgBase + item.backdrop_path} alt={item.name} />
                <div className='absolute bottom-[80px] text-white px-3 pl-[28px] sm:pl-[36px] md:pl-[48px] lg:pl-[60px] w-full'>
                  <h2 className='text-center sm:text-left font-bold text-xl sm:text-3xl md:text-4xl lg:text-6xl xl:text-7xl sm:w-[30ch] mb-3'>{item.name}</h2>
